...
`;
  ...
  if (currentStep === 'auth') {
    ...
    // No repositories found - show manual entry only
    if (!loadingRepos && repositories.length === 0 && user) {
      return (
        <div className="max-w-4xl mx-auto">
          <div className="bg-white p-8 rounded-lg shadow-sm border text-center mb-8">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-4">No Repositories Found</h3>
            <p className="text-lg text-gray-600 mb-8 max-w-2xl mx-auto">
              We couldn't find any repositories in your {user.provider === 'gitlab' ? 'GitLab' : 'GitHub'} account. You can still create a wishlist by entering your repository URL manually below.
            </p>
            <button
              onClick={() => {
              ...
              className="btn-sparkle inline-flex items-center justify-center px-8 py-4"
            >
              <svg className="w-6 h-6 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
              </svg>
              <span>Create Wishlist Manually</span>
            </button>
          </div>
        </div>
      );
